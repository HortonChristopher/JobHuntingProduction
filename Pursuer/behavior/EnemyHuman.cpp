...
void EnemyHuman::Update()
{
	...
	case AGGRO:
		...
		else if (aggroSet)
		{
			...
			if (!FirstRun)
			{
				position.x += 50.0f * (deltaTime->deltaTimeCalculated.count() / 1000000.0f) * (x / hypotenuse);
				position.z += 50.0f * (deltaTime->deltaTimeCalculated.count() / 1000000.0f) * (y / hypotenuse);
			}
			else
			{
				FirstRun = false;
